...
docker build -t rosbot_remote/vnc:latest -f Dockerfile --build-arg RAND=$RANDOM --build-arg BUILD_TYPE=$build_type .


# Get current date in YYYY-MM-DD format
printf -v date '%(%Y-%m-%d)T' -1
# Generate tag
auto_tag="rosbot_remote/vnc:$tag_prefix-$date$tag_postfix"
echo "Adding tag: $auto_tag"

docker tag rosbot_remote/vnc_client:latest  $auto_tag
